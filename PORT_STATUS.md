...
Updated: 2026-08-15. Gates: format ✅ ameba ✅ specs ✅ (2 examples) coverage — (phase 0)
...
- [ ] `http.cr` (connection pool, SigV4 signing, user agent)
- [ ] `retry.cr` (+ specs)
- [ ] `client.cr` — 17 operations
- [ ] `paginator.cr` (+ specs)
- [ ] `waiters.cr` (+ specs)
- [ ] `stub.cr` — `stub_responses` / `api_requests` / `before_request` (+ specs)
- [ ] `spec/aws-record/dynamodb/client_spec.cr` — webmock, exact wire JSON per operation
