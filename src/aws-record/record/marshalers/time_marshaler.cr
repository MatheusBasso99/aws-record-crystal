...
# Marshals `time_attr` attributes.
#
# Values are converted to UTC unless `use_local_time` is set, and stored the way Ruby's
# `Time#iso8601` writes them: `"2009-02-13T23:31:30Z"` for UTC, `"2015-11-15T17:12:56+07:00"`
# otherwise.
class Aws::Record::Marshalers::TimeMarshaler < Aws::Record::Marshalers::Marshaler
  # What `#type_cast` produces.
  alias Cast = Time?

  # Formats a time the way Ruby's `Time#iso8601` does.
  ISO8601 = ->(time : Time) : String { time.utc? ? time.to_s("%FT%TZ") : time.to_s("%FT%T%:z") }
